...
사실 너무 빠르게(1-에포크에서) 0으로 나와 이게 맞는건지 궁금합니다.
1 epoch 진행하는데 몇 시간이 걸리다보니 많이 돌려보지 못했습니다.
-> 추가적으로 알게 된건 제가 전체를 하나로 묶어 출력하다보니 그런 것 같습니다.
왜냐하면 대부분이 태그값 0으로 나오기 때문인 것 같네요... 나눠서 다시 출력해봐야 할 것 같습니다.
그래도 이렇게 느린게 맞나요? 이부분은 궁금하네요 mps를 사용해서 그런건지
